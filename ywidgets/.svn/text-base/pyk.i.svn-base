 ...
 * 
 * $Id: pyk.i,v 1.3-n31 2007/12/21 20:51:45 frigaut Exp $
 * 
...
func killpyk(void) {
  extern GLOB_PYKPAS;
  write, "WARNING : pyk desactivated";
  print_all_dialogs, "WARNING : pyk desactivated";
  GLOB_PYKPAS=1;
}
...
func usepyk(void) {
  ...
  n = GLOB_PYKPAS;
  GLOB_PYKPAS = 0;
  write, swrite(format="WARNING : pyks are alive (%d catched)", n-1);
  print_all_dialogs, swrite(format="WARNING : pyks are alive (%d catched)", n-1);
}

func pyk(py_prefix,py_command)
 ...
 */
{
  ...
  if (!is_array(py_prefix)) return;
  else {
    if (is_array(py_command)) py_command=py_prefix+py_command;
    else py_command=py_prefix;
  }
  
  if (is_void(_pyk_proc)) {
    // start python through pyk.py (includes python initialization):
    pykpy = find_in_path("styc.py",takefirst=1);
...
{
//	"begin callback with : " + line;
//	swrite(format="1 : synchronized %d _pyk_sync %d", synchronized , _pyk_sync);
  extern _pyk_proc;
  if (!line) {
    _pyk_proc = [];
    if (_pyk_sync) resume;
  ...
  //write, "nofline:", nofline;
  if (pyk_debug && nofline) write, "from python:", line;

  /* parse and execute yorick command lines */
  for (i=1 ; i<=nofline ; i++) funcdef(line(i));

  //swrite(format="2 : synchronized %d _pyk_sync %d", synchronized , _pyk_sync);
